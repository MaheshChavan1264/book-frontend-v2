      ...
      axios.get(`${this.baseURL}cart/?token=${this.token}`).then(
        (response) => {
          if (response.status == 200) {
            ...
            let len = Object.keys(books.cartItems).length;
            for (let i = 0; i < len; i++)
              this.checkoutBodyArray.push({
                imageUrl: books.cartItems[i].book.imageURL,
                bookTitle: books.cartItems[i].book.title,
                quantity: books.cartItems[i].quantity,
                price: books.cartItems[i].book.price,
                bookId: books.cartItems[i].book.id,
                userId: books.cartItems[i].userId,
              });
            console.log("checkout",this.checkoutBodyArray)
            console.log("checkout",this.checkoutBodyArray)
          }
        },
        (err) => {
